    ...
    if (CAN_SEE_OBJ(ch, i)) {
      if (cond_top < 50) {
        ...
        if ((ITEM_TYPE(i) == ITEM_TRAP) || (GET_TRAP_CHARGES(i) > 0)) {
          num = number(1, 100);
          if (ch->skills &&
              num < (ch->skills[ SKILL_LOCATE_TRAP ].learned / 2)) {
...
void show_char_to_char(struct char_data *i, struct char_data *ch, int mode) {
  ...
  if (mode == 0) {
    ...
    */
    ...
    if (RIDDEN(i)) {
      sprintf(buffer, "$n is ridden by %s", IS_NPC(RIDDEN(i))
                                                ? RIDDEN(i)->player.short_descr
                                                : GET_NAME(RIDDEN(i)));
      act(buffer, FALSE, i, 0, ch, TO_VICT);
    }
...
void do_look(struct char_data *ch, char *argument, int cmd) {
  ...
  if (IS_AFFECTED(ch, AFF_BLIND))
    ...
    if (keyword_no != 8 && keyword_no != 9 &&
        IS_AFFECTED2(ch, AFF2_SUN_BLIND)) {
      send_to_char(
        ...
        if (*arg2) {
          ...
          if (!found) {
            tmp_desc = find_ex_description(
                arg2, real_roomp(ch->in_room)->ex_description);
            if (tmp_desc) {
...
void do_who(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!IS_IMMORTAL(ch) || cmd == 234) {
      ...
      if (arg[ 0 ] == '-') {
        if (index(arg, '?') != NULL) {
          ...
          send_to_char("[-]d=linkdead g=God o=Mort "
                       "[1]Mage[2]Cleric[3]War[4]Thief[5]Druid[6]Monk\n\r",
                       ch);
          send_to_char("--------\n\r", ch);
        }
        for (person = character_list; person; person = person->next) {
            ...
            if (!skip) {
              ...
              if ((person->desc != NULL) || (index(arg, 'd') != NULL)) {
                for (l = 1; l <= strlen(arg); l++) {
                  ...
                  case 'l': {
                    sprintf(
                        tempbuf, "Level:[%-2d/%-2d/%-2d/%-2d/%-2d/%-2d] ",
    ...
    if (listed == 0) {
      ...
      sprintf(buf, "\n\rTotal players / Link dead [%d/%d] (%2.0f%%) Number "
                   "Listed: %d\n\r",
...
void do_where_object(struct char_data *ch, struct obj_data *obj, int recurse,
                     struct string_block *sb) {
  char buf[ MAX_STRING_LENGTH ];
...
void do_levels(struct char_data *ch, char *argument, int cmd) {
  ...
  */
  ...
  if (!*argument) {
    ...
    if (HasClass(ch, CLASS_CLERIC)) {
      exp = (titles[ CLERIC_LEVEL_IND ][ GET_LEVEL(ch, CLERIC_LEVEL_IND) + 1 ]
                 .exp);
      sprintf(buf,
    ...
    if (HasClass(ch, CLASS_WARRIOR)) {
      exp = titles[ WARRIOR_LEVEL_IND ][ GET_LEVEL(ch, WARRIOR_LEVEL_IND) + 1 ]
                .exp;
      sprintf(buf,
    ...
    for (i = 1, spl = 0; i <= MAX_EXIST_SPELL; i++, spl++) {
      if (GetMaxLevel(ch) > LOW_IMMORTAL ||
        ...
        sprintf(buf + strlen(buf),
                ...
                spells[ spl ], skill_info[ i ].min_usesmana,
...
void do_attribute(struct char_data *ch, char *argument, int cmd) {
  ...
  if ((GetMaxLevel(ch) > 15) ||
      (HasClass(ch, CLASS_MAGIC_USER) || HasClass(ch, CLASS_MONK))) {
...
void do_value(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!SpyCheck(ch)) { /* failed spying check */
    if (obj && vict) {
      act("$n looks at you, and $s eyes linger on $p", FALSE, ch, obj, vict,
          TO_VICT);
  ...
  if (!HasClass(ch, CLASS_THIEF)) { /* it had better be an intrinsic */
    if (GET_RACE(ch) == RACE_DWARF &&
        (GET_ITEM_TYPE(obj) != ITEM_WEAPON &&
         ...
         GET_ITEM_TYPE(obj) != ITEM_ARMOR && GET_ITEM_TYPE(obj) != ITEM_ROCK &&
...
void do_report(struct char_data *ch, char *argument, int cmd) {
  ...
  sprintf(buf, "H:%d/%d    M:%d/%d    V:%d/%d   Exp:%d", GET_HIT(ch),
          GET_MAX_HIT(ch), GET_MANA(ch), GET_MAX_MANA(ch), GET_MOVE(ch),
          GET_MAX_MOVE(ch), GET_EXP(ch));
