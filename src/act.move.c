...
int RawMove(struct char_data *ch, int dir) {
  ...
  if (to_here == NULL) {
    ...
    send_to_char("Uh-oh.  The ground melts beneath you as you fall into the "
                 "swirling chaos.\n\r",
                 ch);
    do_look(ch, "\0", 15);
  ...
  if ((from_here->sector_type == SECT_TREE) ||
    ...
    if (MOUNTED(ch)) {
      if (!IS_AFFECTED(MOUNTED(ch), AFF_TREE_TRAVEL)) {
        send_to_char("Your mount would have to be able to walk through trees "
                     "to go there!\n\r",
                     ch);
        return (FALSE);
      }
    ...
    if (((!IS_AFFECTED(ch, AFF_SNEAK)) || (IS_IMMORTAL(tmp_ch))) &&
      ...
      if (tmp_ch != ch && (!IS_AFFECTED(ch, AFF_SILENCE) || number(0, 2))) {
        if (dir < 4) {
          if (total == 1) {
            if (MOUNTED(ch)) {
              sprintf(tmp, "%s from the %s, riding on %s", PERS(ch, tmp_ch),
                      dirs[ rev_dir[ dir ] ], PERS(MOUNTED(ch), tmp_ch));
            } else {
              sprintf(tmp, "%s %s from the %s.", PERS(ch, tmp_ch), how,
                      dirs[ rev_dir[ dir ] ]);
            }
          } else {
            sprintf(tmp, "%s %s from the %s.", PERS(ch, tmp_ch), how,
