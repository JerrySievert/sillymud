...
void do_backstab(struct char_data *ch, char *argument, int cmd) {
  ...
  if (MOUNTED(ch)) {
    send_to_char("How can you surprise someone with all the racket that beast "
                 "makes?\n\r",
                 ch);
    return;
  }
        ...
        if (RIDDEN(victim) == ch) {
          ...
          if (check > 5) {
            if (RideCheck(ch, -5)) {
              ...
              act("$n gets pissed and $N falls on $S butt!", FALSE, victim, 0,
                  ch, TO_NOTVICT);
              act("$n gets pissed you fall off!", FALSE, victim, 0, ch,
                  TO_VICT);
            }
          } else if (check > 0) {
            act("$n has an indifferent look.", FALSE, victim, 0, 0, TO_ROOM);
...
void do_flee(struct char_data *ch, char *argument, int cmd) {
  ...
  if (IS_PC(ch) || IS_SET(ch->specials.act,
                          ACT_POLYSELF)) { /* This only works on PC types */
    ...
    do {
      for (i = 0; i < 3; i++) {
        ...
        if (CAN_GO(ch, attempt) &&
          ...
          found = TRUE;
          switch (attempt) { /* Character will not try to flee back the way he
                                came after first attempt to flee */
          case 0:
            badroom = 2;
            break;
          case 1:
            ...
            for (j = 0; j < 6; j++) {
              if (CAN_GO(ch, j) &&
                  !IS_SET(real_roomp(EXIT(ch, j)->to_room)->room_flags,
                          DEATH)) {
                attempt = j;
                switch (attempt) { /* Since retreat picks a new exit, we gotta
                                      badroom test again */
                case 0:
                  badroom = 2;
                  break;
      ...
      tries++;
    } while ((!found) && (tries < (ch->specials.flee))); /* This keeps the
                                                            sucker fighting if
                                                            he didn't escape */
      ...
      for (f = 0; f < ((ch->specials.flee) - tries); f++) {
        for (i = 0; i < 6; i++) {
          ...
          if (CAN_GO(ch, attempt) &&
            ...
            badroom = -1; /* This is so that after you leave this room, you
                             won't know or have prejudices against
  ...
  if (((!IS_NPC(ch)) && (IS_SET(ch->specials.act, PLR_WIMPY))) ||
    ...
    if (strlen(buf) != 0) {
      ...
      if (IS_NPC(ch))
        REMOVE_BIT(
            ch->specials.act,
            ACT_WIMPY); /* eld - For polys... I'm surprised it wasn't in here
                           originally */
      else
        REMOVE_BIT(ch->specials.act, PLR_WIMPY);
      send_to_char("Ok, you are no longer a wimp...\n\r", ch);
      ch->specials.pct = pct;
      sprintf(buf2, "And you will now get your BLEEDING message at %d%% of max "
                    "hitpoints.\n\r",
              ch->specials.pct);
      send_to_char(buf2, ch);
    } else {
      if (IS_NPC(ch))
        ...
        REMOVE_BIT(ch->specials.act, PLR_WIMPY);
      send_to_char("Ok, you are no longer a wimp...\n\r", ch);
      sprintf(buf2, "However, you will still get your BLEEDING message at %d%% "
                    "of max hitpoints.\n\r",
              ch->specials.pct);
      send_to_char(buf2, ch);
    }
...
void kick_messages(struct char_data *ch, struct char_data *victim, int damage) {
  ...
  if (!damage) {
    act(att_kick_miss_ch[ i ], FALSE, ch, ch->equipment[ WIELD ], victim,
        TO_CHAR);
    ...
    act(att_kick_miss_room[ i ], FALSE, ch, ch->equipment[ WIELD ], victim,
        TO_NOTVICT);
             ...
             -10) {
    act(att_kick_kill_ch[ i ], FALSE, ch, ch->equipment[ WIELD ], victim,
        TO_CHAR);
    ...
    act(att_kick_hit_ch[ i ], FALSE, ch, ch->equipment[ WIELD ], victim,
        TO_CHAR);
