  ...
  if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
    ...
    if (AWAKE(victim) && (percent > ch->skills[ SKILL_STEAL ].learned)) {
      ...
      if (ch->skills[ SKILL_STEAL ].learned < 90) {
        act("Even though you were caught, you realize your mistake and promise "
            "to remember.",
      ...
      if (!HasClass(ch, CLASS_MAGIC_USER) && !HasClass(ch, CLASS_CLERIC) &&
        ...
        for (i = 0; *spells[ i ] != '\n'; i++)
          if (skill_info[ i + 1 ].spell_pointer &&
              (ch->skills[ i + 1 ].learned || IsIntrinsic(ch, i + 1))) {
  ...
  case 'm': {
    ...
    for (i = 0; *spells[ i ] != '\n'; i++)
      if (skill_info[ i + 1 ].spell_pointer &&
        ...
        sprintf(buf, "[%d] %s %s \n\r",
                skill_info[ i + 1 ].min_level[ MIN_LEVEL_MAGIC ], spells[ i ],
                how_good(ch->skills[ i + 1 ].learned));
        if (strlen(buf) + strlen(buffer) > (MAX_STRING_LENGTH * 2) - 2)
          break;
        strcat(buffer, buf);
  ...
  case 'p': {
    ...
    for (i = 0; *spells[ i ] != '\n'; i++)
      if (skill_info[ i + 1 ].spell_pointer &&
        ...
        sprintf(buf, "[%d] %s %s \n\r",
                skill_info[ i + 1 ].min_level[ MIN_LEVEL_CLERIC ], spells[ i ],
                how_good(ch->skills[ i + 1 ].learned));
        if (strlen(buf) + strlen(buffer) > (MAX_STRING_LENGTH * 2) - 2)
          break;
        strcat(buffer, buf);
  ...
  case 'd': {
    ...
    for (i = 0; *spells[ i ] != '\n'; i++)
      if (skill_info[ i + 1 ].spell_pointer &&
        ...
        sprintf(buf, "[%d] %s %s \n\r",
                skill_info[ i + 1 ].min_level[ MIN_LEVEL_DRUID ], spells[ i ],
                how_good(ch->skills[ i + 1 ].learned));
  ...
  if (!*name) {
    if (!IS_AFFECTED(ch, AFF_GROUP)) {
      ...
      if (IS_AFFECTED(k, AFF_GROUP)) {
        sprintf(buf, "     $N (Head of group) HP:%s MV:%s", Condition(k),
                Tiredness(k));
      ...
      for (f = k->followers; f; f = f->next)
        if (IS_AFFECTED(f->follower, AFF_GROUP)) {
          sprintf(buf, "     $N  \tHP:%s MV:%s", Condition(f->follower),
                  Tiredness(f->follower));
...
void do_recite(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!HasClass(ch, CLASS_MAGIC_USER) && !HasClass(ch, CLASS_CLERIC)) {
    if (ch->skills[ SKILL_READ_MAGIC ].learned < number(1, 95)) {
      send_to_char("after several seconds of study, you realize that you can't "
                   "understand this\n\r",
                   ch);
...
void do_use(struct char_data *ch, char *argument, int cmd) {
  ...
  if (stick->obj_flags.type_flag == ITEM_STAFF) {
    ...
    for (fol = ch->followers; fol; fol = fol->next)
      if (IS_SET(fol->follower->specials.act, ACT_FIGURINE)) {
        send_to_char("The beast you attempted to summon will not come to this "
                     "plane of existance.\n",
                     ch);
        return;
      }
    ...
    if (GET_HIT(stick->link) < GET_MAX_HIT(stick->link) / 7) {
      send_to_char("The beast refuses to step foot into this plane of "
                   "existance until it has healed.\n\r",
                   ch);
      return;
    }
    stick = unequip_char(ch, HOLD);
  ...
  if (cmd == 276 || cmd == 278) {
    ...
    if (IsRideable(horse)) {
      ...
      if (check > 5) {
        ...
        act("as $n tries to mount $N, $N attacks $n!", FALSE, ch, 0, horse,
            TO_NOTVICT);
        WAIT_STATE(ch, PULSE_VIOLENCE);
        hit(horse, ch, TYPE_UNDEFINED);
        return;
      } else if (check > -1) {
        act("$N moves out of the way, you fall on your butt", FALSE, ch, 0,
            horse, TO_CHAR);
        act("as $n tries to mount $N, $N moves out of the way", FALSE, ch, 0,
            horse, TO_NOTVICT);
        WAIT_STATE(ch, PULSE_VIOLENCE);
      ...
      if (RideCheck(ch, 50)) {
        ...
        act("You try to ride $N, but falls on $s butt", FALSE, ch, 0, horse,
            TO_CHAR);
        act("$n tries to ride $N, but falls on $s butt", FALSE, ch, 0, horse,
            TO_NOTVICT);
        act("$n tries to ride you, but falls on $s butt", FALSE, ch, 0, horse,
            TO_VICT);
        GET_POS(ch) = POSITION_SITTING;
        WAIT_STATE(ch, PULSE_VIOLENCE * 2);
      }
...
void do_donate(struct char_data *ch, char *argument, int cmd) {
  ...
  if (count > 1) {
    ...
    send_to_room(buf, 99);
    act("Thank you for your most generous donation.", FALSE, ch, 0, 0, TO_CHAR);
  } else if (count == 1) {
...
void do_auto(struct char_data *ch, char *argument, int cmd) {
  ...
  int keyword_no;
  static char *keywords[] = { "loot", "split", "", "\n" };
