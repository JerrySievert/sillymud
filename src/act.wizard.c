  ...
  */
  ...
  if (*argument) {
    ...
    if (str_cmp(word, "yes") == 0) {
      ...
      sprintf(buf2, "%s has just accepted %s into the game.", ch->player.name,
              name);
      debug(buf2);
      SEND_TO_Q("You have been accepted.  Press enter\n\r", d);
    } else if (str_cmp(word, "no") == 0) {
      SEND_TO_Q("You have been denied.  Press enter\n\r", d);
      sprintf(buf2, "%s has just denied %s from the game.", ch->player.name,
              name);
      debug(buf2);
...
void do_goto(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!real_roomp(location)) {
    debug("Massive error in do_goto. Everyone Off NOW.");
    return;
  }
  ...
  if (!*arg1) {
    ...
    if (k = get_char_vis_world(ch, arg1, &count)) {
      ...
      sprintf(buf2, "   Level [%d/%d/%d/%d/%d/%d/%d] Alignment[%d]\n\r",
              ...
              k->player.level[ 3 ], k->player.level[ 4 ], k->player.level[ 5 ],
      ...
      sprintf(buf, "Age: [%d] Y, [%d] M, [%d] D, [%d] H. ", ma.year, ma.month,
              ma.day, ma.hours);
      send_to_char(buf, ch);
      ...
      sprintf(buf, "Mana:[%d/%d+%d] Hit:[%d/%d] Move:[%d/%d+%d]\n\r",
              ...
              hit_limit(k), GET_MOVE(k), move_limit(k), move_gain(k));
      send_to_char(buf, ch);

      sprintf(buf, "AC:[%d/10], Coins: [%d], Exp: [%d], Hitroll: [%d], "
                   "Damroll: [%d] sf[%d]\n\r",
      ...
      sprintf(buf, "Carried weight: %d   Carried items: %d ", IS_CARRYING_W(k),
              IS_CARRYING_N(k));
    ...
    if (j = (struct obj_data *)get_obj_vis_world(ch, arg1, &count)) {
      ...
      sprintf(buf,
              ...
              j->obj_flags.timer, GET_OBJ_EGO(j));
      send_to_char(buf, ch);
...
void do_return(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!ch->desc->original) {
    ...
    send_to_char("You pause for a moment and remold your flesh into it's "
                 "original shape.\n\r",
                 ch);
  ...
  do {
    for (i = 0; i < MAX_STAT; i++) {
      ...
      temp = (unsigned int)rools[ 0 ] + (unsigned int)rools[ 1 ] +
             ...
             MIN((int)rools[ 0 ],
                 MIN((int)rools[ 1 ], MIN((int)rools[ 2 ], (int)rools[ 3 ])));
