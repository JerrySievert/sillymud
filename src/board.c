...
char save_file[ NUM_BOARDS ][ 20 ] = { "mortal.board", "wiz.board",
                                       "skexie.board" };
...
1.1 changes:
   ...
   constants NUM_BOARDS and add the new V-Numbers to the if/then structure
directly
...
void board_write_msg(struct char_data *ch, char *arg, int bnum) {
  ...
  if (bnum == -1) {
    debug("Board special procedure called for non-board object.\r\n");
  ...
  if (GetMaxLevel(ch) < min_write_level[ bnum ]) {
    send_to_char("You pick up a quill to write, but realize you're not "
                 "powerful enough\n\r",
                 ch);
  ...
  if (!*arg) {
    send_to_char("The board has now been saved permanently to disk.\n\rTo "
                 "write a new message, use WRITE followed by a title.\n\r",
                 ch);
    return;
  }
...
int board_remove_msg(struct char_data *ch, char *arg, int bnum) {
  ...
  debug(buf); /* Message removals now logged. */
...
void board_save_board(int bnum) {
  ...
  if (!the_file) {
    debug("Unable to open/create savefile for bulletin board..\n\r");
    return;
  }
...
void board_load_board( ) {
  ...
  for (bnum = 0; bnum < NUM_BOARDS; bnum++) {
    ...
    if (boards[ bnum ].number < 0 || boards[ bnum ].number > MAX_MSGS ||
        feof(the_file)) {
      debug("Board-message file corrupt, nonexistent, or empty.\n\r");
      boards[ bnum ].number = -1;
      fclose(the_file);
      continue;
    }
...
int board_show_board(struct char_data *ch, char *arg, int bnum) {
  ...
  strcpy(buf, "This is a bulletin board. Usage: READ/REMOVE <messg #>, WRITE "
              "<header>\n\r");
  if (boards[ bnum ].number == -1)
...
int fwrite_string(char *buf, FILE *fl) { return (fprintf(fl, "%s~\n", buf)); }
...
int board_check_locks(int bnum, struct char_data *ch) {
  ...
  if (!found) {
    debug("Board: board locked for a user not in game.");
    board_lock[ bnum ].lock       = 0;
