  ...
  while ((pos < argc) && (*(argv[ pos ]) == '-')) {
    ...
    case 'd':
      if (*(argv[ pos ] + 2))
        ...
        debug("Directory arg expected after option -d.");
        assert(0);
      }
      break;
    case 's':
      no_specials = 1;
      debug("Suppressing assignment of special routines.");
      break;
    default:
...
#if SITELOCK
  debug("Blanking denied hosts.");
  for (a = 0; a <= MAX_BAN_HOSTS; a++)
    strcpy(hostlist[ a ], " \0\0\0\0");
  numberhosts = 0;
#if LOCKGROVE
  debug("Locking out Host: oak.grove.iup.edu.");
  strcpy(hostlist[ 0 ], "oak.grove.iup.edu");
  numberhosts = 1;
  debug("Locking out Host: everest.rutgers.edu.");
...
void run_the_game(int port) {
  ...
  debug("Signal trapping.");
  signal_setup( );

  debug("Opening mother connection.");
  s = init_socket(port);

  if (lawful && load( ) >= 6) {
    debug("System load too high at startup.");
    coma(1);
  }
  ...
  debug("Normal termination of game.");
}
...
void write_to_q(char *txt, struct txt_q *queue) {
  ...
  if (!queue) {
    debug("Output message to non-existant queue");
    return;
  }
  ...
  if (strl < 0 || strl > 15000) {
    debug("strlen returned bogus length in write_to_q");
    free(new);
    return;
  }
...
int new_descriptor(int s) {
  ...
  *newd->host = '\0';
  ...
  if (*newd->host == '\0') {
    inet_ntop(AF_INET, &(sock.sin_addr), buf, INET_ADDRSTRLEN);
    strcpy(newd->host, buf);
  }
  ...
  do {
    if ((thisround = read(t->descriptor, t->buf + begin + sofar,
      ...
      if (thisround < 0) {
        ...
        debug("EOF encountered on socket read.");
        return (-1);
      }
    }
...
void close_socket(struct descriptor_data *d) {
  ...
  if (d->character)
    ...
    debug("Losing descriptor without char.");
...
void coma(int s) {
  ...
  do {
    ...
    tics = 1;
    if (workhours( )) {
      debug("Working hours collision during coma. Exit.");
      assert(0);
    }
  } while (load( ) >= 6);
