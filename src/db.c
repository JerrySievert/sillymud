...
void boot_db( ) {
  ...
  debug("Resetting the game time:");
  reset_time( );

  debug("Reading newsfile, credits, help-page, info and motd.");
  ...
  debug("Initializing Script Files.");
  ...
  CommandSetup( );
  InitScripts( );
  debug("Opening mobile, object and help files.");
  ...
  if (!(help_fl = fopen(HELP_KWRD_FILE, "r")))
    debug("   Could not open help file.");
  else
    help_index = build_help_index(help_fl, &top_of_helpt);

  debug("Booting Figurine Table.");
  BootFigurines( );

  debug("Loading zone table.");
  boot_zones( );

  debug("Loading rooms.");
  boot_world( );

  debug("Generating index tables for mobile and object files.");
  mob_index = generate_indices(mob_f, &top_of_mobt);
  obj_index = generate_indices(obj_f, &top_of_objt);

  debug("Renumbering zone table.");
  renum_zone_table( );
#if 0
	debug("Cleaning up player file.");
	clean_playerfile();
#endif
  debug("Generating player index.");
  build_player_index( );

  debug("Loading fight messages.");
  load_messages( );

  debug("Loading social messages.");
  boot_social_messages( );

  debug("Loading pose messages.");
  boot_pose_messages( );

  debug("Assigning function pointers:");
  if (!no_specials) {
    debug("   Mobiles.");
    assign_mobiles( );
    debug("   Objects.");
  ...
  debug("Updating characters with saved items:");
  update_obj_file( );
  debug("Loading saved rooms.");
  ReloadRooms( );
...
int intcomp(struct wizs *j, struct wizs *k) { return (k->level - j->level); }
...
void build_player_index( ) {
    ...
    if (!feof(fl)) /* new record */ {
      ...
      for (i = 0; i <= 5; i++)
        if (dummy.level[ i ] >= 51) {
          ...
          list_wiz.lookup[ max - 51 ]
              .stuff[ list_wiz.number[ max - 51 ] ]
              .name = (char *)strdup(dummy.name);
          list_wiz.lookup[ max - 51 ]
              .stuff[ list_wiz.number[ max - 51 ] ]
              .title = (char *)strdup(dummy.title);
  ...
  debug("Began Wizlist Generation.");
  ...
  for (;;) {
    if (fgets(buf, sizeof(buf), fl)) {
      if (*buf == '#') {
        ...
        index[ i ].func   = 0;
        index[ i ].name =
            (index[ i ].virtual < 99999) ? fread_string(fl) : strdup("omega");
        i++;
      } else {
        if (*buf == '$') /* EOF */
          break;
...
void completely_cleanout_room(struct room_data *rp) {
  ...
  while (rp->people) {
    ch = rp->people;
    act("The hand of god sweeps across the land and you are swept into the "
        "Void.",
...
void boot_world( ) {
  ...
  if (!(fl = fopen(WORLD_FILE, "r"))) {
    perror("fopen");
    debug("boot_world: could not open world file.");
    assert(0);
  }
  ...
  for (i = 0; i < top_of_scripts; i++) {
    if (script_data[ i ].virtual == mob_index[ nr ].virtual) {
      ...
      /* sprintf(buffer, "Setting SCRIPT bit for mobile %s, file %s.",
         GET_NAME(mob), script_data[i].filename);
         debug(buffer); */
      mob->script = i;
      break;
    }
...
void reset_zone(int zone) {
  ...
  if (zone == 0 && !done) {
    ...
    for (i = 0; i < 30000; i += 1000) {
      ...
      fl = fopen(buf, "r");
      if (!fl) {
        debug("Unable to load scratch zone file for update.");
        return;
      }
      ReadTextZone(fl);
      fclose(fl);
          ...
          if (GET_RACE(mob) > RACE_GNOME)
            if (!strchr(zone_table[ ZCMD.arg1 ].races, GET_RACE(mob))) {
              zone_table[ ZCMD.arg1 ]
                  .races[ strlen(zone_table[ ZCMD.arg1 ].races) ] =
                  GET_RACE(mob);
            }
        }
        break;
...
void char_to_store(struct char_data *ch, struct char_file_u *st) {
  ...
  if ((i >= MAX_AFFECT) && af && af->next)
    debug("WARNING: OUT OF STORE ROOM FOR AFFECTED TYPES!!!");
...
void reset_char(struct char_data *ch) {
  ...
  } else if (HasClass(ch, CLASS_THIEF) || HasClass(ch, CLASS_WARRIOR) ||
...
void reboot_text(struct char_data *ch, char *arg, int cmd) {
  ...
  debug("Rebooting Essential Text Files.");
...
void InitScripts( ) {
  ...
  if (!(f1 = fopen("scripts.dat", "r"))) {
    debug("Unable to open file \"scripts.dat\".");
    return;
  }
  ...
  while (1) {
    ...
    if (!(f2 = fopen(buf, "r"))) {
      ...
      script_data = (struct scripts *)realloc(
          script_data, (top_of_scripts + 1) * sizeof(struct scripts));
...
void SaveTheWorld( ) {
  ...
  if (!fp) {
    debug("Unable to open zone writing file.");
    return;
  }
...
void BootFigurines( ) {
  ...
  if (!(f1 = fopen("figurines", "r"))) {
    debug("Unable to open file \"figurines\".");
    return;
  }
