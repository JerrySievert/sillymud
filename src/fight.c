...
void load_messages( ) {
  ...
  while (*chk == 'M') {
    ...
    if (i >= MAX_MESSAGES) {
      debug("Too many combat messages.");
      exit(0);
    }
...
void set_fighting(struct char_data *ch, struct char_data *vict) {
  ...
  if (ch->specials.fighting) {
    debug("Fighting character set to fighting another.");
    return;
  }
  ...
  if (vict->attackers <= 5) {
    ...
    debug("more than 6 people attacking one target");
  }
...
void stop_fighting(struct char_data *ch) {
  ...
  if (ch->specials.fighting->attackers < 0) {
    debug("too few people attacking");
  ...
  if (combat_list == ch)
    ...
    for (tmp = combat_list; tmp && (tmp->next_fighting != ch);
      ...
      ;
    if (!tmp) {
      debug("Char fighting not found Error (fight.c, stop_fighting)");
      abort( );
    }
...
void DamageMessages(struct char_data *ch, struct char_data *v, int dam,
  ...
  case POSITION_MORTALLYW:
    act("$n is mortally wounded, and will die soon, if not aided.", TRUE, v, 0,
        0, TO_ROOM);
    act("You are mortally wounded, and will die soon, if not aided.", FALSE, v,
        0, 0, TO_CHAR);
    break;
  case POSITION_INCAP:
  ...
  case POSITION_STUNNED:
    act("$n is stunned, but will probably regain consciousness again.", TRUE, v,
        0, 0, TO_ROOM);
  ...
  default: /* >= POSITION SLEEPING */
    ...
    if (GET_HIT(v) < (max_hit / (100. / (v->specials.pct))) &&
        GET_HIT(v) > 0) { /* Wimpy pct coding */
      act("You wish that your wounds would stop BLEEDING so much!", FALSE, v, 0,
          0, TO_CHAR);
      ...
      if (victim->in_room > -1) {
        if (IS_NPC(ch) && !IS_SET(ch->specials.act, ACT_POLYSELF)) {
          ...
          if ((IS_GOOD(ch) && !IS_EVIL(victim)) ||
            ...
            sprintf(buf, "%s killed by %s at %s", GET_NAME(victim),
                    GET_NAME(ch), (real_roomp(victim->in_room))->name);
          }
        }
      } else {
      ...
      if (IS_SET(ch->specials.affected_by2, AFF2_FEEDING)) {
        if (ch->in_room != ch->specials.fighting->in_room) {
          ...
          if (ch->attackers > 1) {
            ...
            send_to_char("You can no longer feed with all these people "
                         "fighting you.\n\r",
            ...
            if ((perc = number(1, 101)) <
              ...
              if (number(1, 20) > GET_DEX(ch)) {
                ...
                if (number(1, 20) > GET_DEX(ch)) {
                  tmp = unequip_char(ch, WIELD);
                  obj_to_room(tmp, ch->in_room);
                  act("and you fumble and drop $p too!", 0, ch, tmp, tmp,
                      TO_CHAR);
                  act("and then fumbles and drops $p as well!", 0, ch, tmp, tmp,
                      TO_ROOM);
                }
              }
            }
...
struct char_data *FindVictim(struct char_data *ch) {
  ...
  unsigned short ftot = 0, ttot = 0, ctot = 0, ntot = 0, mtot = 0, ktot = 0,
                 dtot = 0;
    ...
    if ((CAN_SEE(ch, tmp_ch)) &&
      ...
      if (!IS_SET(ch->specials.act, ACT_WIMPY) || !AWAKE(tmp_ch)) {
        if ((tmp_ch->specials.zone != ch->specials.zone &&
          ...
          /*
                  if (!IS_NPC(tmp_ch)||(IS_SET(tmp_ch->specials.act,
             ACT_ANNOYING))) {
          */
  ...
  if (type >= TYPE_HIT && type <= TYPE_SMITE) {
    ...
    if (v->equipment[ num ]) {
      if ((type == TYPE_BLUDGEON && dam > 10) ||
          (type == TYPE_CRUSH && dam > 5) || (type == TYPE_SMASH && dam > 10) ||
          (type == TYPE_BITE && dam > 15) || (type == TYPE_CLAW && dam > 20) ||
    ...
    if ((c->equipment[ WIELD ]) &&
      ...
      for (j = 0; j < MAX_OBJ_AFFECT; j++) {
        if (c->equipment[ WIELD ]->affected[ j ].location ==
          ...
          ((*skill_info[ num ].spell_pointer)(6, c, "", SPELL_TYPE_WAND, v, 0));
        }
      }
    }
  }
...
int MonkDodge(struct char_data *ch, struct char_data *v, int *dam) {
  ...
  if (number(1, 20000) <
      v->skills[ SKILL_DODGE ].learned * GET_LEVEL(v, MONK_LEVEL_IND)) {
    *dam = 0;
