...
void change_alignment(struct char_data *ch, struct char_data *victim) {
  ...
  if (HasClass(ch, CLASS_DRUID)) {
    ...
    if (d2 > diff) {
      ...
      if (d2 > 150) {
        ...
        if (d2 > 275) {
          send_to_char(
              "If you do not mend your ways soon, you will be punished\n\r",
              ch);
          if (d2 > 425) {
  ...
  if (HasClass(ch, CLASS_DRUID)) {
    if (GET_ALIGNMENT(ch) > 600 || GET_ALIGNMENT(ch) < -600) {
      send_to_char("Soylent Green, god of druids, has excommunicated you for "
                   "your heresies\n\r",
                   ch);
      ...
      if (AWAKE(ch) && (ch->in_room == ch->specials.fighting->in_room) &&
        ...
        if (!IS_NPC(ch)) {
          ...
          if (!HasClass(ch, CLASS_WARRIOR)) {
            if (HasClass(ch, CLASS_CLERIC) || HasClass(ch, CLASS_THIEF)) {
