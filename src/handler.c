...
void affect_modify(struct char_data *ch, byte loc, long mod, long bitv,
  ...
  default:
    debug("Unknown apply adjust attempt (handler.c, affect_modify).");
    debug(ch->player.name);
...
void affect_remove(struct char_data *ch, struct affected_type *af) {
  ...
  if (!ch->affected) {
    debug("affect removed from char without affect");
    debug(GET_NAME(ch));
    return;
  }
  ...
  if (ch->affected == af) {
    ...
    if (hjp->next != af) {
      debug("Could not locate affected_type in ch->affected. (handler.c, "
            "affect_remove)");
      return;
    }
...
void char_from_room(struct char_data *ch) {
  ...
  if (ch->in_room == NOWHERE) {
    debug("NOWHERE extracting char from room (handler.c, char_from_room)");
    return;
  }
...
void obj_from_char(struct obj_data *object) {
  ...
  if (!object) {
    debug("No object to be take from char.");
    assert(0);
  }

  if (!object->carried_by) {
    debug("this object is not carried by anyone");
    assert(0);
  }

  if (!object->carried_by->carrying) {
    debug("No one is carrying this object");
    assert(0);
  }

  if (object->in_obj) {
    debug("Obj in more than one place.");
    assert(0);
  }
  ...
  if (object->carried_by->carrying == object) /* head of list */
    ...
    if (!tmp) {
      debug("Couldn't find object on character");
      assert(0);
    }
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
  ...
  if (obj->carried_by) {
    debug("EQUIP: Obj is carried_by when equip.");
    assert(0);
  }

  if (obj->in_room != NOWHERE) {
    debug("EQUIP: Obj is in_room when equip.");
    assert(0);
    return;
  }
  ...
  if (j < -5) {
    ...
    act("$p almost seems to say 'You're pretty puny.  I don't want to be seen "
        "with you!\n",
        0, ch, obj, 0, TO_CHAR);
    act("$p falls to the floor", 0, ch, obj, 0, TO_CHAR);
    act("$p removes itself, and falls to the floor", 0, ch, obj, 0, TO_ROOM);
  ...
  if (ItemAlignClash(ch, obj)) {
    if (ch->in_room != NOWHERE) {
      ...
      debug("ch->in_room = NOWHERE when equipping char.");
      assert(0);
    }
  }
  ...
  for (j = 0; j < MAX_OBJ_AFFECT; j++)
    affect_modify(ch, obj->affected[ j ].location,
                  (int)obj->affected[ j ].modifier, obj->obj_flags.bitvector,
                  TRUE);
...
void obj_from_room(struct obj_data *object) {
  ...
  if (object->in_room <= NOWHERE) {
    if (object->carried_by || object->equipped_by) {
      debug("Eek.. an object was just taken from a char, instead of a room");
      assert(0);
    }
  ...
  else /* locate previous element in list */
  {
    ...
    if (i) {
      ...
      debug("Couldn't find object in room");
      assert(0);
    }
  }
...
void extract_obj(struct obj_data *obj) {
  ...
  if (obj->in_room != NOWHERE)
    ...
    if (obj->eq_pos > -1) {
      ...
      debug(obj->equipped_by->player.name);
      debug(obj->name);
      return;
    }
  } else if (obj->in_obj) {
  ...
  if (object_list == obj) /* head of list */
    ...
    if (temp1) {
      ...
      debug("Couldn't find object in object list.");
      assert(0);
    }
  }
...
void extract_char(struct char_data *ch) { extract_char_smarter(ch, NOWHERE); }
...
void extract_char_smarter(struct char_data *ch, int save_room) {
  ...
  if (ch->in_room == NOWHERE) {
    debug("NOWHERE extracting char. (handler.c, extract_char)");
    /*
  ...
  if (ch == character_list)
    ...
    if (k)
      ...
      debug("Trying to remove ?? from character_list.(handler.c,extract_char)");
      exit(0);
    }
  }
...
struct obj_data *create_money(int amount) {
  ...
  if (amount <= 0) {
    debug("ERROR: Try to create negative money.");
    assert(0);
  }
  ...
  for (i = 0; i < MAX_OBJ_AFFECT; i++) {
    if (o->affected[ i ].location != APPLY_NONE) {
      affect_modify(ch, o->affected[ i ].location,
                    (int)o->affected[ i ].modifier, o->obj_flags.bitvector,
                    TRUE);
    } else {
      return;
    }
