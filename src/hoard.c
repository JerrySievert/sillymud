...
int No_Hoard(struct char_data *ch, struct char_data *re, struct obj_data *obj,
  ...
  while (obj) {
    ...
    if ((newflags & 2) && (newflags == 2)) {
      ...
      ; /* This will clear up TAKE */
    else if (newflags == 16384)
      ; /* This will clear up TAKE, HOLD's */
    else {
    ...
    if ((obj->obj_flags.type_flag == 15) && (flag == 0)) {
      ...
      return (
          1); /* Anyone burying shit in bags within bags is a hoarder by my
                 book! */
  ...
  for (i = 0; i < MAX_WEAR; i++) {
    ...
    while (obj) {
      ...
      if ((obj->obj_flags.type_flag == 15) && (flag == 0)) {
        ...
        return (1); /* Anyone burying shit in bags within bags is a hoarder by
                       my book! */
      } else if (!(obj->next_content)) {
        if (flag == 2) {
          obj  = obj2->next_content;
  ...
  for (i = 0; i < misfits;
       i++) { /* Now to see if the multi-wears fit anywhere */
