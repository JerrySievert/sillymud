...
void command_interpreter(struct char_data *ch, char *argument) {
  ...
  if (!IS_NPC(ch)) {
    ...
    if ((!ch->player.name[ 0 ]) || (ch->player.name[ 0 ] < ' ')) {
      debug("Error in character name.  Changed to 'Error'");
      free(ch->player.name);
...
void nanny(struct descriptor_data *d, char *arg) {
    ...
    if (!*arg) {
        ...
        if (is_number(arg)) {
          ...
          if (choice < 1 || choice > i) {
            ...
            GET_RACE(d->character) =
                RaceList[ choice ].race_num; /* for CVC() */
      ...
      if (choice < 1 || choice > i) {
        ...
        if (!get_racial_alignment(d)) { /* neutral chars can choose */
          SEND_TO_Q("Shall you start with Good, Neutral, or Evil (G/N/E) "
                    "tendencies? ",
                    d);
          STATE(d) = CON_ALIGN;
          break;
  ...
  case CON_QCLASS: {
    ...
    for (; *arg && count < 3 && !oops; arg++) {
      ...
      case '?':
        ...
        SEND_TO_Q("Magic-users:  Weak, puny, smart and very powerful at high "
                  "levels.\n\r",
                  d);
        SEND_TO_Q(
            "Thieves:      Quick, agile, sneaky.  Nobody trusts them.\n\r", d);
        SEND_TO_Q("Monks:        Masters of the martial arts.  They can only "
                  "be single classed\n\r",
                  d);
        SEND_TO_Q("\n\r", d);
        DisplayRaceClasses(d);
      ...
      */
      ...
      if (!strncmp(d->host, "oak.grove", 9) ||
        ...
        if (top_of_p_table > 0) {
          ...
          debug("Initial character.  Authorized Automatically");
  ...
  case CON_PWDNCNF:
    ...
    break;
  default:
    debug("Nanny: illegal state of con'ness");
    abort( );
    break;
  }
}

char *class_selections[] = { "(M)age",  "(C)leric", "(W)arrior", "(T)hief",
                             "(D)ruid", "mon(K)",   "\n" };
...
int CheckValidClass(struct descriptor_data *d, int class) {
  ...
  if (class == CLASS_MAGIC_USER &&
      RacialMax[ GET_RACE(d->character) ][ 0 ] > 10)
    return (TRUE);
