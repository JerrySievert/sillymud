...
struct PolyType DruidList[ 17 ] = {
  { "bear", 10, 9024 },   { "spider", 10, 20010 },
  { "lamia", 10, 3648 },  { "lizard", 10, 6822 },
  { "bear", 12, 9056 }, /* 5 */
  { "gator", 12, 9054 },  { "basilisk", 13, 7043 },
  { "snog", 14, 27008 },  { "snake", 15, 6517 },
  { "spider", 15, 6113 }, /* 10 */
  { "lizard", 16, 6505 }, { "allosaurus", 18, 21801 },
  { "tiger", 28, 9027 },  { "mulichort", 30, 15830 },
  { "tiger", 35, 9055 },                              /* 15 */
  { "lion", 35, 13718 },  { "salamander", 35, 25506 } /* 17 */
};
  ...
  if (*arg) {
    ...
    if (victim) {
      if (GetMaxLevel(victim) >= LOW_IMMORTAL) {
        ...
        act("$n contemplates feeding on $mself but decides against it.", TRUE,
            ch, 0, victim, TO_ROOM);
        return;
...
void MindflayerAttack(struct char_data *ch, struct char_data *v) {
  ...
  if (v->equipment[ WEAR_HEAD ]) { /* hey, this bozo has headgear! */
    ...
    if (number(1, 10) >= (v->equipment[ WEAR_HEAD ]->obj_flags.value[ 0 ])) {
      ...
      act("Your tentacles are blocked by $N's headgear!", FALSE, ch, 0, v,
          TO_CHAR);
      act("$N's tentacles are blocked by $n's headgear!", FALSE, ch, 0, v,
          TO_NOTVICT);
      return;
    }
  }
  ...
  if (HitOrMiss(ch, v, CalcThaco(ch)) && GetMaxLevel(v) < LOW_IMMORTAL) {
    /* ha ha, we suck them */
    act("$N screams out in pain as $n's tentacles suck $S energies!", FALSE, ch,
        0, v, TO_NOTVICT);
    act("$n's tentacles momentarily wrap themselves around your head!", FALSE,
        ch, 0, v, TO_VICT);
    send_to_char("Blinding pain fills your head, and flashes of red dot your "
                 "vision.\n\r",
                 v);
    ...
    if (GET_MANA(v) > 0) {
      ...
      act("Blech, $E has no more magical energies for you to drain.", FALSE, ch,
          0, v, TO_CHAR);
    }
...
void VampireBite(struct char_data *ch, struct char_data *v) {
  ...
  if (v->specials.bitten) { /* vampire's victim */
    ...
    if (chance > number(1, 100)) {
      act("$N struggles violently and breaks free of $n's grasp!", FALSE, ch, 0,
          v, TO_NOTVICT);
      act("$N struggles violently and breaks free of your grasp!", FALSE, ch, 0,
          v, TO_CHAR);
      act("You struggle violently and break free of $n's grasp!", FALSE, ch, 0,
          v, TO_VICT);
    ...
    if (HitOrMiss(ch, v, CalcThaco(ch))) {
      ...
      to_bite = FALSE;
      act("$n bares $s long, sharp fangs and almost bites you!", FALSE, ch, 0,
          v, TO_VICT);
  ...
  if (to_bite) {
    ...
    if (GET_HIT(v) > 0) {
      act("You savor the salty taste of $N's blood as you drain it.", FALSE, ch,
          0, v, TO_CHAR);
