...
void advance_level(struct char_data *ch, int class) {
  ...
  if (class > MAX_CLASS) {
    debug("Bad advance class.. no such class");
    return;
  }
  ...
  if (ch->specials.spells_to_learn < 50) {
    if (GET_RACE(ch) == RACE_OGRE)
      ch->specials.spells_to_learn +=
          MIN(3, MAX(1, MAX(2, wis_app[ (int) GET_RWIS(ch) ].bonus) /
                            HowManyClasses(ch)));
    else
      ch->specials.spells_to_learn +=
...
void drop_level(struct char_data *ch, int class) {
  ...
  ch->points.exp =
      MIN(titles[ (int) lin_class ][ (int) GET_LEVEL(ch, lin_class) ].exp, GET_EXP(ch));
        ...
        for (i = MAGE_LEVEL_IND; i < MAX_CLASS; i++) {
          ...
          if (GET_LEVEL(ch, i) &&
            ...
            if (GET_EXP(ch) >= titles[ i ][ GET_LEVEL(ch, i) + 2 ].exp) {
              send_to_char("You must practice at a guild before you can gain "
                           "any more experience\n\r",
                           ch);
...
void gain_exp_regardless(struct char_data *ch, int gain, int class) {
  ...
  if (!IS_NPC(ch)) {
    if (gain > 0) {
      ...
      for (i = 0;
           (i < ABS_MAX_LVL) && (titles[ class ][ i ].exp <= GET_EXP(ch));
           i++) {
...
void ObjFromCorpse(struct obj_data *c) {
  ...
  for (jj = c->contains; jj; jj = next_thing) {
    ...
    if (jj->in_obj) {
       ...
       */
      c->contains = 0;
      debug("Memory lost in ObjFromCorpse.");
      return;
    }
  }
  extract_obj(c);
