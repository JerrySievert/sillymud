...
void spell_shocking_grasp(byte level, struct char_data *ch,
  ...
  if ((GET_HIT(victim) < -4) && IsHumanoid(victim) && !IsUndead(victim)) {
    act("$n utters the words 'clear', and touches $N's chest!!", FALSE, ch, 0,
        victim, TO_ROOM);
    GET_HIT(victim) += dam;
    update_pos(victim);
    return;
...
void spell_fireball(byte level, struct char_data *ch, struct char_data *victim,
  ...
  AreaDamage(ch, dam, SPELL_FIREBALL, "", "You dodge the mass of flame!!\n\r",
    ...
    if ((ch->in_room == tmp_victim->in_room) && (ch != tmp_victim)) {
      if (!in_group(ch, tmp_victim) && !IS_IMMORTAL(tmp_victim)) {
        ...
        if (GetMaxLevel(tmp_victim) > 4) {
          ...
          act("You are sucked into a huge hole in the ground!", FALSE, ch, 0,
              tmp_victim, TO_VICT);
          act("$N is sucked into a huge hole in the ground!", FALSE, ch, 0,
...
void spell_dispel_evil(byte level, struct char_data *ch,
  ...
  if (IsExtraPlanar(victim)) {
    ...
    act("$N laughs at you for thinking it was extra-planar.", TRUE, ch, 0,
        victim, TO_CHAR);
...
void spell_armor(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (IS_NEUTRAL(ch) || (IS_GOOD(ch) && !IS_EVIL(victim)) ||
    ...
    act("$n calls upon $s god to help $N, but the god refuses!", TRUE, ch, 0,
        victim, TO_NOTVICT);
...
void spell_bless(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (obj) {
    ...
    if ((GET_POS(victim) != POSITION_FIGHTING) &&
      ...
      if (IS_EVIL(ch)) {
        if (ch != victim)
          act("$n bestows an unholy blessing upon $N.", TRUE, ch, 0, victim,
              TO_NOTVICT);
        else
        ...
        if (ch != victim)
          act("$n bestows a holy blessing upon $N.", TRUE, ch, 0, victim,
              TO_NOTVICT);
        else
...
void spell_blindness(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (saves_spell(victim, SAVING_SPELL)) {
    ...
    if (ch != victim) {
      act("$N blinks twice, and ignores $n's attempt to blind $M.", TRUE, ch, 0,
          victim, TO_ROOM);
...
void spell_cure_critic(byte level, struct char_data *ch,
  ...
  if (ch != victim) {
    ...
    if (IS_EVIL(ch))
      act("$n's hands glow red as $e lays hands on $N", TRUE, ch, 0, victim,
          TO_NOTVICT);
    else if (IS_GOOD(ch))
      act("$n's hands glow white as $e lays hands on $N", TRUE, ch, 0, victim,
          TO_NOTVICT);
    else /* must be neutral */
      act("$n's hands glow blue as $e lays hands on $N", TRUE, ch, 0, victim,
          TO_NOTVICT);
  } else {
    if (IS_EVIL(ch))
      act("$n's wounds strongly glow red and are healed!", TRUE, ch, 0, victim,
          TO_NOTVICT);
    else if (IS_GOOD(ch))
      act("$n's wounds strongly glow white and are healed!", TRUE, ch, 0,
          victim, TO_NOTVICT);
    else /* must be neutral */
      act("$n's wounds strongly glow blue and are healed!", TRUE, ch, 0, victim,
          TO_NOTVICT);
  }
...
void spell_cure_light(byte level, struct char_data *ch,
  ...
  if (ch != victim) {
    act("You make $N feel much better.", TRUE, ch, 0, victim, TO_CHAR);
    act("$n recites a soothing prayer for $N", TRUE, ch, 0, victim, TO_NOTVICT);
  } else {
    if (IS_EVIL(ch))
      act("$n's wounds faintly glow red and are healed!", TRUE, ch, 0, victim,
          TO_NOTVICT);
    else if (IS_GOOD(ch))
      act("$n's wounds faintly glow white and are healed!", TRUE, ch, 0, victim,
          TO_NOTVICT);
    else /* must be neutral */
      act("$n's wounds faintly glow blue and are healed!", TRUE, ch, 0, victim,
          TO_NOTVICT);
  }
  update_pos(victim);
...
void spell_curse(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (obj) {
    ...
    act("$p glows polkadotted.  (very bad indeed)", FALSE, ch, obj, 0, TO_CHAR);
  } else { /* must be a victim */
    if (saves_spell(victim, SAVING_SPELL)) {
      act("$N resists your attempt to curse $M.", FALSE, ch, 0, victim,
          TO_CHAR);
      return;
    }
    ...
    if (ch != victim) {
      ...
      act("$n curses $N, $E is surrounded by a malignant aura!", TRUE, ch, 0,
          victim, TO_NOTVICT);
      act("$n curses you.  You are enveloped in a malignant aura.", TRUE, ch, 0,
          victim, TO_VICT);
...
void spell_poison(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (victim) {
    ...
    if (IsImmune(victim, IMM_POISON) ||
        (!IS_SET(ch->specials.act, ACT_DEADLY) &&
         GET_RACE(ch) == RACE_VEGMAN)) {
      act("$N tried to poison you, but you ignore the poison!", TRUE, victim, 0,
          ch, TO_CHAR);
      act("$n tried to poison $N, but $E ignores it!", TRUE, ch, 0, victim,
          TO_NOTVICT);
    ...
    if (IS_SET(ch->specials.act, ACT_DEADLY) && ch != victim) {
      ...
      if (IsResist(victim, IMM_POISON)) {
        ...
        if (!ImpSaveSpell(victim, SAVING_PARA, 0)) {
          ...
          act("$n tries to poison $N, but $E resists it!", TRUE, ch, 0, victim,
              TO_NOTVICT);
      ...
      if (!ImpSaveSpell(victim, SAVING_PARA, 0)) {
        ...
        act("$n fills $N's veins with deadly poison!", TRUE, ch, 0, victim,
            TO_NOTVICT);
        ...
        act("Deadly poison fills your veins, luckily you resist.", TRUE, victim,
            0, 0, TO_CHAR);
      ...
      if (IsResist(victim, IMM_POISON)) {
        if (ImpSaveSpell(victim, SAVING_PARA, 0)) {
          act("$N tried to poison you, but you resist it!", TRUE, victim, 0, ch,
              TO_CHAR);
          act("$n tries to poison $N, but $E resists it!", TRUE, ch, 0, victim,
              TO_NOTVICT);
      ...
      if (!ImpSaveSpell(victim, SAVING_PARA, 0)) {
        ...
        act("$n tries to poison $N, but it fails!", TRUE, ch, 0, victim,
            TO_NOTVICT);
...
void spell_summon(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (!IS_PC(victim)) {
    count = 0;
    for (tmp = real_roomp(victim->in_room)->people; tmp;
    ...
    if (count == 0) {
      ...
      for (tmp = real_roomp(victim->in_room)->people; count && tmp;
