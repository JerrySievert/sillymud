...
/* check for the ONE_LIFE bit.  if it is set, go through the
   ...
   */
        ...
        st.talks[ 2 ] = TRUE;
        st.abilities.con -= 1;
        act("A clear bell rings throughout the heavens", TRUE, ch, 0, 0,
            TO_CHAR);
        act("A ghostly spirit smiles, and says 'Thank you'", TRUE, ch, 0, 0,
            TO_CHAR);
        act("A clear bell rings throughout the heavens", TRUE, ch, 0, 0,
            TO_ROOM);
        act("A ghostly spirit smiles, and says 'Thank you'", TRUE, ch, 0, 0,
            TO_ROOM);
        act("$p dissappears in the blink of an eye.", TRUE, ch, obj, 0,
            TO_ROOM);
        act("$p dissappears in the blink of an eye.", TRUE, ch, obj, 0,
            TO_ROOM);
        GET_MANA(ch) = 1;
        ...
        send_to_char("The body does not have the strength to be recreated.\n\r",
                     ch);
      }
      fclose(fl);
    }
      ...
      if ((number(1, 100)) <= chance) {
        for (i = 0; i < diff; i++) {
          if ((!saves_spell(victim, SAVING_SPELL)) &&
            ...
            act("$n turns $N into a smoldering pile of dust!", TRUE, ch, 0,
                victim, TO_NOTVICT);
            act("You disintegrate $N!!!", TRUE, ch, 0, victim, TO_CHAR);
            act("$n has just undone your life force!", TRUE, ch, 0, victim,
                TO_VICT);
...
void spell_holy_word(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (IS_GOOD(ch)) { /* If the caster is GOOD */
    send_to_char("You feel righteous fury fill your body as you declare your "
                 "holy word!\n\r",
                 ch);
    act("$n bellows a holy word!  You stagger from the blast and shield your "
        "eyes!",
        TRUE, ch, 0, victim, TO_ROOM);
  }

  if (IS_EVIL(ch)) { /* If the caster is EVIL */
    send_to_char("You feel demonic power fill your soul as you declare your "
                 "unholy word!\n\r",
                 ch);
  ...
  for (t = real_roomp(ch->in_room)->people; t; t = next) {
    ...
    if (!in_group(ch, t) && !IS_IMMORTAL(t)) {
      if (((IS_GOOD(ch)) && (!IS_EVIL(t))) ||
          ...
          ((!IS_EVIL(ch)) && (!IS_GOOD(ch)) && (!IS_EVIL(t)) &&
           (!IS_GOOD(t)))) {
        ...
        act("$N is allready in awe of your holy wrath!", TRUE, ch, 0, t,
            TO_CHAR);
      }
    }
  }
...
void holy_banish(byte level, struct char_data *ch, struct char_data *victim,
                 struct obj_data *obj) {
  act("$n forces you from this plane.", 0, ch, 0, victim, TO_VICT);
  act("$N screams as $n banishes $M from this plane!", TRUE, ch, 0, victim,
      TO_ROOM);
...
void holy_destroy(byte level, struct char_data *ch, struct char_data *victim,
                  struct obj_data *obj) {
  act("$N is destroyed in the searing blast!!", TRUE, ch, 0, victim,
      TO_NOTVICT);
...
void holy_bigdif(byte level, struct char_data *ch, struct char_data *victim,
                 struct obj_data *obj) {
  struct affected_type af;
  int damg = 1;
  ...
  act("$N screams in agony and terror at $n's furious blast!", TRUE, ch, 0,
      victim, TO_NOTVICT);
  act("Your word blasts $N, $E screams in terror and pain!", 0, ch, 0, victim,
      TO_CHAR);
  act("You scream out as $n blasts you. You are paralyzed with fear!", 0, ch, 0,
      victim, TO_VICT);
...
void holy_meddif(byte level, struct char_data *ch, struct char_data *victim,
                 struct obj_data *obj) {
  struct affected_type af;
  int damg = 1;
  ...
  act("$N is blasted by $n's mighty word, bringing $N to $S knees.", TRUE, ch,
      0, victim, TO_NOTVICT);
  act("You blast $N with your power, making $M writhe in silent agony.", 0, ch,
      0, victim, TO_CHAR);
  act("You are knocked to your knees as $n's word blasts your spirit", 0, ch, 0,
      victim, TO_VICT);
...
void holy_smldif(byte level, struct char_data *ch, struct char_data *victim,
                 struct obj_data *obj) {
...
void spell_poly_self(byte level, struct char_data *ch, struct char_data *mob,
  ...
  mob->specials.pct =
      ch->specials.pct; /* Copy wimpy percentage so that it isn't 0 */
  ...
  GET_NAME(mob) = buf;
  buf = (char *)malloc(strlen(mob->player.short_descr) + strlen(GET_NAME(ch)) +
                       2);
...
void spell_shield(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (!affected_by_spell(victim, SPELL_SHIELD)) {
    ...
    if (ch != victim) {
      ...
      act("You are surrounded by a strong force shield.", TRUE, ch, 0, 0,
          TO_CHAR);
    }
...
void spell_Create_Monster(byte level, struct char_data *ch,
  ...
  if (too_many_followers(ch)) {
    act("$N says 'No way I'm hanging with that crowd!!'", TRUE, ch, 0, mob,
        TO_ROOM);
    act("$N refuses to hang out with crowd of your size!!", TRUE, ch, 0, mob,
        TO_CHAR);
  } else {
...
void spell_web(byte level, struct char_data *ch, struct char_data *victim,
  ...
  if (fail) {
    ...
    if (!pissed) {
      ...
      act("You are wrapped in webs, but they don't stop you!", FALSE, ch, 0,
          victim, TO_VICT);
      act("$N attacks, paying little heed to the webs that slow it.", FALSE, ch,
          0, victim, TO_NOTVICT);
      act("You only manage to piss off $N with your webs, ack!", FALSE, ch, 0,
          victim, TO_CHAR);
    }
  } else {
    if (pissed) {
      act("You are almost caught in a sticky webbing, GRRRR!", FALSE, ch, 0,
          victim, TO_VICT);
      act("$N growls and dodges $n's sticky webbing!", FALSE, ch, 0, victim,
          TO_NOTVICT);
      act("You miss $N with your sticky webbing!  Uh oh, it's mad.", FALSE, ch,
          0, victim, TO_CHAR);
    } else {
      act("You watch with amusement as $n casts web about the room.", FALSE, ch,
          0, victim, TO_VICT);
      act("$n misses $N with the webs!", FALSE, ch, 0, victim, TO_NOTVICT);
      act("You miss with your webs, but $N doesn't seem to notice.", FALSE, ch,
          0, victim, TO_CHAR);
    }
  }
  if (pissed)
...
void spell_conjure_elemental(byte level, struct char_data *ch,
  ...
  */
  ...
  if (too_many_followers(ch)) {
    act("$N says 'No way I'm hanging with that crowd, but thanks for the lift'",
        TRUE, ch, 0, victim, TO_ROOM);
    act("$N refuses to hang out with crowd of your size, but thanks you for "
        "the free ride.",
        TRUE, ch, 0, victim, TO_CHAR);
  } else {
      ...
      if (!in_group(ch, tmp_victim)) {
        if (IS_AFFECTED(tmp_victim, AFF_INVISIBLE)) {
          if (saves_spell(tmp_victim, SAVING_SPELL)) {
            ...
            act("$n is briefly revealed, but dissapears again.", TRUE,
                tmp_victim, 0, 0, TO_ROOM);
            act("You are briefly revealed, but dissapear again.", TRUE,
...
void spell_cacaodemon(byte level, struct char_data *ch,
  ...
  if (too_many_followers(ch)) {
    act("$N says 'No way I'm hanging with that crowd!!'", TRUE, ch, 0, victim,
        TO_ROOM);
    act("$N refuses to hang out with crowd of your size!!", TRUE, ch, 0, victim,
        TO_CHAR);
  } else {
...
void spell_geyser(byte level, struct char_data *ch, struct char_data *victim,
  ...
  AreaDamage(ch, dam, SPELL_GEYSER, "You are seared by the boiling water!!\n\r",
