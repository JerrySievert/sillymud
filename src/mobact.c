      ...
      if (ch->specials.hunting) {
        if (ch->specials.hunting->in_room == ch->in_room) {
          if (Hates(ch, ch->specials.hunting) &&
            ...
            if (check_peaceful(ch, "You'd love to tear your quarry to bits, "
                                   "but you just CAN'T\n\r")) {
...
#endif
    debug("Char not in correct room.  moving to 50 ");
    debug(GET_NAME(ch));
  ...
  if (AWAKE(ch) && (!ch->specials.fighting) && (!ch->desc) &&
    ...
    if (GET_HIT(ch) > (GET_MAX_HIT(ch) / 2)) {
      if (IS_SET(ch->specials.act, ACT_HATEFUL)) {
        ...
        if (tmp_ch) {
          if (check_peaceful(ch, "You ask your mortal enemy to step outside to "
                                 "settle matters.\n\r")) {
            act("$n growls '$N, would you care to step outside where we can "
                "settle this?'",
          ...
          if (IS_SET(ch->specials.act, ACT_HATEFUL)) {
            ...
            if (tmp_ch) {
              if (check_peaceful(ch, "You ask your mortal enemy to step "
                ...
                act("$n growls '$N, would you care to step outside where we "
                    "can settle this?'",
                    TRUE, ch, 0, tmp_ch, TO_ROOM);
              } else {
                if (IsHumanoid(ch)) {
...
void end2(char *arg, struct char_data *ch) { ch->commandp = 0; }
