  ...
  if (obj) {
	...
	send_to_char(buf, ch);
      ...
      if (ItemEgoClash(ch, obj, 0) < 0 && obj->obj_flags.cost_per_day > 0) {
        if (re)
          act("$p refuses to be rented with a wimp like you!", TRUE, ch, obj, 0,
              TO_CHAR);
        cost->ok = FALSE;
      } else if (cost->ok) {
        if (re) {
...
#if NEW_RENT
    timegold =
        (int)((100 * ((float)time(0) - st.last_update)) / (SECS_PER_REAL_DAY));
#else
	    ...
	    WriteObjs(fl, &st);
            ...
            if (days_passed > 0) {
              ...
              if ((st.total_cost * days_passed) > st.gold_left) {
                ...
                ch_st.load_room   = NOWHERE;
                fseek(char_file,
                      (long)(player_table[ i ].nr * sizeof(struct char_file_u)),
                      0);
                fwrite(&ch_st, sizeof(struct char_file_u), 1, char_file);
    ...
    if (st->objects[ i ].item_number > -1 &&
      ...
      */
      if (cost_per_day > LIM_ITEM_COST_MIN) {
        ...
        if (IS_OBJ_STAT(obj, ITEM_MAGIC) || IS_OBJ_STAT(obj, ITEM_GLOW) ||
            IS_OBJ_STAT(obj, ITEM_HUM) || IS_OBJ_STAT(obj, ITEM_INVISIBLE) ||
