...
void do_inset(struct char_data *ch, char *argument, int cmd) {
  ...
  bits = generic_find(sp, FIND_OBJ_INV, ch, &tmp_char, &sword);

  bits = generic_find(sp2, FIND_OBJ_INV, ch, &tmp_char, &gem);
...
/*  static struct hash_header	x_room; */
  ...
  if ((predicate)(in_room, c_data))
   ...
   if (top_of_world > MAX_ROOMS) {
     debug("TRACK Is disabled, too many rooms.\n\rContact Loki soon.\n\r");
    return -1;
   }
#endif
              ...
              if (CAN_WEAR(obj_object, ITEM_TAKE)) {
                if (!IS_SET(sub_object->obj_flags.value[ 1 ], CONT_CLOSED)) {
                  ...
                  if (obj_object->obj_flags.type_flag == ITEM_MONEY) {
                    ...
                    obj_from_char(obj_object);
                    sprintf(buffer, "There %s %d coins.\n\r",
                            obj_object->obj_flags.value[ 0 ] > 1 ? "were"
                                                                 : "was",
                            obj_object->obj_flags.value[ 0 ]);
                    send_to_char(buffer, ch);
                    GET_GOLD(ch) += obj_object->obj_flags.value[ 0 ];
...
void do_berserk(struct char_data *ch, char *arg, int cmd) {
  ...
  if (!ch->specials.fighting) {
    act("$n gets worked up into a lather and runs madly about!", TRUE, ch, 0, 0,
        TO_ROOM);
...
#define CAULDRON 1531
  ...
  for (max = 0, j = 1; j < 4; j++) {
    if (potion->obj_flags.value[ j ] >= 1) {
      for (i = 0; i < MIN_LEVEL_NUM; i++)
        if (skill_info[ potion->obj_flags.value[ j ] ].min_level[ i ] <
            LOW_IMMORTAL)
          max = MAX(max,
  ...
  if (ingredients != i || match != 0) {
    ...
    act("$n accidentally lets a drop of sweat fall into the brew...", FALSE, ch,
        0, 0, TO_ROOM);
    act("You stood too close to the cauldron.  A drop of your sweat just fell "
        "in...",
        FALSE, ch, 0, 0, TO_CHAR);
    act("The cauldron overflows!  Everything is ruined!", FALSE, ch, 0, 0,
        TO_ROOM);
    act("Next time pay more attention to your master... You just ruined this "
        "batch!",
        FALSE, ch, 0, 0, TO_CHAR);
  } else {
    potion = read_object(BrewList[ which_potion ].object[ 5 ], VIRTUAL);
    ...
    act("$n carefully mixes some secret ingredients in a cauldron.", FALSE, ch,
        0, 0, TO_ROOM);
    act("You carefully mix the ingredients of an age-old recipe.", FALSE, ch, 0,
        0, TO_CHAR);
    act("Suddenly... Rainbows shoot out of the pot, and sparks fly!", FALSE, ch,
        0, 0, TO_ROOM);
    act("Then in a burst of light, nothing remains but a bright flask!", FALSE,
        ch, 0, 0, TO_ROOM);
    act("Suddenly... Rainbows shoot out of the pot, and sparks fly!", FALSE, ch,
        0, 0, TO_CHAR);
    act("Then in a burst of light, nothing remains but a bright flask!", FALSE,
  ...
  for (in_o = o->contains; in_o; in_o = next) {
    next = in_o->next_content;
    obj_from_obj(in_o);
    act("$p has vanished in a flash of bright light.", FALSE, ch, in_o, 0,
        TO_CHAR);
    act("$p has vanished in a flash of bright light.", FALSE, ch, in_o, 0,
        TO_ROOM);
    extract_obj(in_o);
...
void add_skill(int nr, int taught_by, int class_use, int percent) {
  ...
  skill_info[ nr ].min_level[ MIN_LEVEL_CLERIC ] =
      LOKI + 1; /* changed by Kiku */
  skill_info[ nr ].min_level[ MIN_LEVEL_MAGIC ] =
      LOKI + 1; /* New data structure */
  skill_info[ nr ].min_level[ MIN_LEVEL_DRUID ] =
      LOKI + 1; /* implementation. */
