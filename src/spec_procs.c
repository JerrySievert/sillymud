...
int monk_master(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  if (!HasClass(ch, CLASS_MONK))
    return (
        Teacher(ch, cmd, arg, mob, type, TAUGHT_BY_MONK, "monk guildmaster"));
  else
...
int Guildmaster(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type, int class) {
  ...
  if (HasClass(ch, class)) {
        ...
        for (i = 0; *spells[ i ] != '\n'; i++) {
          if (skill_info[ i + 1 ].spell_pointer &&
            ...
            sprintf(buf, "[%-2d] %-30s %s \n\r",
                    skill_info[ i + 1 ].min_level[ level_num ], spells[ i ],
    ...
    if (ch->skills[ number ].learned >= skill_info[ number ].percent) {
      if (ch->skills[ number ].learned < 95) {
        send_to_char("You must use this skill to get any better.  I cannot "
                     "train you further.\n\r",
                     ch);
      } else {
...
struct pub_beers sold_here[] = { { 3903, 3902, 6, 1 }, { 3905, 3904, 6, 1 },
                                 { 3907, 3906, 6, 1 }, { 3909, 3908, 6, 3 },
                                 { 3911, 3910, 6, 3 }, { 3913, 3912, 6, 3 },
                                 { 3914, 0, 0, 1 },    { 3930, 0, 0, 0 },
                                 { 3931, 0, 0, 0 },    { 3932, 0, 0, 0 },
                                 { 3102, 0, 0, 0 },    { -1 } };

int andy_wilcox(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type)
...
#define THE_PUB 3940
  ...
  case 156: /* steal */
    if (andy == ch)
      return TRUE;
    do_action(andy, ch->player.name, 130 /* slap */);
    act("$n tells you 'Who the hell do you think you are?'", FALSE, andy, 0, ch,
        TO_VICT);
  ...
  case 56: /* buy */
    if (ch->in_room != THE_PUB) {
      act("$n tells you 'Hey man, I'm on my own time, but stop by the Pub some "
          "time.'",
    ...
    if (scan->actflag & ACT_OVER_21 && GET_AGE(ch) < 21) {
      if (IS_IMMORTAL(ch) || HasClass(ch, CLASS_THIEF)) {
        ...
        act("$n tells you 'Sorry, I could lose my license if I served you "
            "alcohol.'",
            FALSE, andy, 0, ch, TO_VICT);
      ...
      if (eric->in_room != Erics_Lair) {
        ...
        if (3 == dice(1, 5)) {
          ...
          case 5:
            ...
            if (temp1 == NULL || temp1->obj_flags.type_flag != ITEM_DRINKCON ||
      ...
      if (eric->in_room != andy->in_room) {
        ...
        for (scan = shopping_list; *scan; scan++) {
          if (NULL == get_obj_in_list_vis(eric, *scan, eric->carrying)) {
            ...
            command_interpreter(eric, buf);
            if (NULL == get_obj_in_list_vis(eric, *scan, eric->carrying)) {
              act("$n says 'ARGH, where's my deadbeat roommate with the rent.'",
                  FALSE, eric, 0, 0, TO_ROOM);
    ...
    case E_STOCK_FRIDGE:
      if (eric->in_room != DanjerKitchen) {
        ...
        if (temp1 == NULL) {
          ...
          command_interpreter(eric, buf);
          if (NULL == get_obj_in_list_vis(eric, "beer", eric->carrying)) {
            act("$n says 'What the hell, I just bought this?!'", FALSE, eric, 0,
                0, TO_ROOM);
            do_drop(eric, "sixpack", -1 /* irrelevant */);
            if (NULL == get_obj_in_list_vis(eric, "sixpack", eric->carrying))
              state = E_HACKING;
        ...
        if (IS_AFFECTED(most_hurt, AFF_TEAM_GREY)) {
          ...
          if (!check_soundproof(ch)) {
            if (number(0, 4) == 0) {
              ...
              act("$n utters the words 'judicandus dies'.", 1, ch, 0, 0,
                  TO_ROOM);
...
int WizardGuard(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
int green_slime(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
int guild_guard(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
int puff(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
  ...
  if (type == EVENT_DWARVES_STRIKE) {
    do_shout(ch, "Ack! Of all the stupid things! Those damned dwarves are on "
                 "strike again!",
             0);
    return (TRUE);
  }
  ...
  if (type == EVENT_DEATHROOM) {
    if (number(0, 1))
      do_shout(
          ch, "Oh boy, cosmic forces reveal a spontaneous influx of items!", 0);
    else
...
int regenerator(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
int RustMonster(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  struct char_data *vict;
...
int Ringwraith(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
  ...
  if (ch->act_ptr == 0) { /* does our ringwraith have his state info? */
    ch->act_ptr = (struct wraith_hunt *)malloc(
        sizeof(*wh)); /* this will never get freed :( */
    wh             = (struct wraith_hunt *)ch->act_ptr;
    wh->ringnumber = 0;
  } else
    wh = (struct wraith_hunt *)
             ch->act_ptr; /* yuck, talk to loki about the act_ptr */
  ...
  if ((victim = char_holding(ring))) {
    ...
    debug("a One Ring was completely disconnected!?");
    wh->ringnumber = 0;
  }
  return TRUE;
}

int WarrenGuard(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  ...
  case POSITION_STANDING:
    ...
    if (temp1) {
      if (GET_MANA(zmaster) < ZM_MANA) {
        ...
        if (!check_soundproof(ch)) {
          act("$n says 'Wonderful, another loyal follower!' and grins "
              "maniacly.",
...
int bank(struct char_data *ch, int cmd, char *arg, struct room_data *rp,
  ...
  if (GET_BANK(ch) > GetMaxLevel(ch) * 40000 && GetMaxLevel(ch) < 40) {
    send_to_char("I'm sorry, but we can no longer hold more than 40000 coins "
                 "per level.\n\r",
                 ch);
  ...
  if (cmd == 219) {
    ...
    if (money > GET_GOLD(ch)) {
      ...
      return (TRUE);
               ...
               (GetMaxLevel(ch) < 40)) {
      send_to_char("I'm sorry, Regulations only allow us to ensure 40000 coins "
                   "per level.\n\r",
                   ch);
      return (TRUE);
    } else {
    ...
    if (cmd <= 6 && cmd >= 1 && IS_PC(ch)) {
      ...
      if ((ch->in_room == Ivory_Gate) && (cmd == 4)) {
        if ((SISYPHUS_MAX_LEVEL < GetMaxLevel(ch)) &&
          ...
          if (!check_soundproof(ch)) {
            act("$n tells you 'First you'll have to get past me!'", TRUE, mob,
                0, ch, TO_VICT);
          }
...
int jabberwocky(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  if (cmd)
    return (FALSE);
...
int jugglernaut(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  ...
  if (GET_POS(ch) == POSITION_STANDING) {
    ...
    if (random( ) % 6) {
      if (random( ) % 2) {
        act("$n tosses $p high into the air and catches it.", TRUE, ch, tmp_obj,
            NULL, TO_ROOM);
      } else {
...
int Manticore(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
              int type) { return FALSE; }
...
int SultanGuard(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
int magic_user2(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  struct char_data *vict;
  byte lspell;
    ...
    if (!IS_SET(obj->obj_flags.extra_flags, ITEM_NODROP)) {
      if (GetMaxLevel(ch) <= MAX_MORT) {
        act("You try to give $p to $N, but it vanishes!", FALSE, ch, obj, t,
            TO_CHAR);
        act("$N tries to give $p to you, but it fades away!", FALSE, t, obj, ch,
            TO_CHAR);
  ...
  if (!cmd) {
    ...
    case Lattimore_Lockers:
      ...
      if (time_info.hours == 17) {
        ...
        act("$n looks frightened, and dives under the nearest bunk.", FALSE, ch,
            0, 0, TO_ROOM);
        strcpy(ch->player.long_descr, lattimore_descs[ 3 ]);
        mem->pointer = Lattimore_Hiding;
      }
    ...
    case Lattimore_Eating:
      ...
      if (time_info.hours > 18) {
        ...
        act("$n gets some bread from the oven to go with his stew.", FALSE, ch,
            0, 0, TO_ROOM);
    ...
    case Lattimore_Run:
      ...
      if (ch->in_room != Storeroom && ch->in_room != Trap) {
        ...
        if (!IS_AFFECTED(ch, AFF_FLYING)) {
          ...
          act("$n grins evilly, and quickly stands on a barrel.", FALSE, ch, 0,
              0, TO_ROOM);
          ...
          if (!IS_NPC(t) && CAN_SEE(ch, t))
            if (!(strcmp(mem->names[ mem->index ], GET_NAME(t)))) {
              ...
              if ((IS_CARRYING_N(t) + 1) < CAN_CARRY_N(t)) {
                ...
                act("$n emerges with $p, and drops it for $N.", FALSE, ch, obj,
                    t, TO_ROOM);
                obj_to_room(obj, ch->in_room);
              }
            }
...
#define Med_Chambers 21324
    ...
    if (gave_this_click) {
      ...
      if (obj_index[ obj->item_number ].virtual == Shield) {
        if (!check_soundproof(ch)) {
          ...
          act("$N says 'You may now undertake the first quest.'", FALSE, ch, 0,
              vict, TO_CHAR);
        }
  ...
  case Valik_Qone:
    if (gave_this_click)
      if (obj_index[ obj->item_number ].virtual == Ring) {
        if (!check_soundproof(ch)) {
          act("$N says 'You have brought me the ring of Tlanic.'", FALSE, ch, 0,
              vict, TO_CHAR);
          act("$N says 'You may now undertake the second quest.'", FALSE, ch, 0,
              vict, TO_CHAR);
        }
        obj_from_char(obj);
        obj_to_char(obj, vict);
      ...
      if (obj_index[ obj->item_number ].virtual == Chalice) {
        if (!check_soundproof(ch)) {
          act("$N says 'You have brought me the chalice of Evistar.'", FALSE,
              ch, 0, vict, TO_CHAR);
          act("$N says 'You may now undertake the third quest.'", FALSE, ch, 0,
              vict, TO_CHAR);
        }
        obj_from_char(obj);
        obj_to_char(obj, vict);
      ...
      if (obj_index[ obj->item_number ].virtual == Circlet) {
        if (!check_soundproof(ch)) {
          act("$N says 'You have brought me the circlet of C*zarnak.'", FALSE,
              ch, 0, vict, TO_CHAR);
          act("$N says 'You may now undertake the final quest.'", FALSE, ch, 0,
              vict, TO_CHAR);
        }
        obj_from_char(obj);
        obj_to_char(obj, vict);
  ...
  case Valik_Qfour:
    if (vict->equipment[ WEAR_NECK_1 ] &&
      ...
      act("$N takes the Necklace of Wisdom and hands it to you.", FALSE, ch, 0,
          vict, TO_CHAR);
    ...
    break;
  default:
    debug("Ack! Foo! Heimdall screws up!");
    return (FALSE);
  }
}
...
#define Necklace 21122
  ...
  if (!cmd && (ch->generic == -1))
    ...
    if (!(pass = fopen(RHYODIN_FILE, "r"))) {
      debug("Rhyodin access file unreadable or non-existant");
      ch->generic = -1;
      return (FALSE);
    }
      ...
      if (obj_index[ obj->item_number ].virtual == Necklace) {
        ...
        if (!IS_NPC(ch)) {
          if (!(pass = fopen(RHYODIN_FILE, "a"))) {
            debug("Couldn't open file for writing permanent Rhyodin passlist.");
            return (FALSE);
          }
...
int web_slinger(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  struct char_data *vict;
