        ...
        if ((ch->in_room == Bandits_Path) && (cmd == 1)) {
          if ((BASIL_GATEKEEPER_MAX_LEVEL < GetMaxLevel(ch)) &&
            ...
            if (!check_soundproof(ch)) {
              act("Basil the Great Druid tells you 'Begone Unbelievers!'", TRUE,
                  ch, 0, 0, TO_CHAR);
            }
...
int cleric(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
  ...
  if (number(1, healperc + 2) > 3) {
    ...
    if (IS_AFFECTED(ch, AFF_POISON) && (lspell >= 5) && (number(0, 6) == 0)) {
      act("$n utters the words 'Praise <Deity Name> I don't feel sick no "
          "more!'.",
          1, ch, 0, 0, TO_ROOM);
...
int Teacher(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
  ...
  if ((cmd == 164) || (cmd == 170)) {
    ...
    if ((number == SKILL_DUAL_WIELD) &&
        (ch->skills[ SKILL_DODGE ].learned < 80) &&
  ...
  if (cmd) {
    ...
    if (cmd == 207) { /* recite */
      ...
      if (strncmp("'word of recall'", p, strlen(p) - 1) == 0) {
        ...
        act("$n says 'Do you think you can astral walk in and out of here like "
            "the ",
      ...
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
      act("$n says 'Your ignorance is too immense to be rectified at this "
          "time. Mensch.'",
          FALSE, Sammy, 0, 0, TO_ROOM);
      return (TRUE);
      ...
      if ((obj_index[ obj->item_number ].func) &&
        ...
        if (holder->equipment[ WIELD ]) {
          if ((!EgoBladeSave(holder)) && (!EgoBladeSave(holder))) {
            send_to_char("The black blade gets pissed off that you are "
        ...
        if (affected_by_spell(holder, SPELL_CHARM_PERSON)) {
          ...
          send_to_char("Due to the black blade, you feel less enthused about "
                       "your master.\n\r",
                       holder);
        }
      }
        ...
        if (holder->specials.fighting) {
          ...
          if ((holder == ch) && (cmd == 151)) {
            if (EgoBladeSave(ch) && EgoBladeSave(ch)) {
              send_to_char("You can feel the black blade attempt to stay in "
                           "the fight!\n\r",
                           ch);
              return (FALSE);
            } else {
              send_to_char("The black blade laughs at your attempt to flee "
                           "from a fight!!\n\r",
                           ch);
              ...
              send_to_char("The black blade twists around and smacks you!\n\r",
                           ch);
              act("Wow! $n's black blade just whipped around and smacked $m "
                  "one!",
...
void SayHello(struct char_data *ch, struct char_data *t) {
  ...
  case 10: {
    ...
    case SKY_CLOUDLESS:
      sprintf(buf, "Lovely weather we're having this %s, isn't it, %s.", buf2,
              GET_NAME(t));
    case SKY_CLOUDY:
    ...
    case SKY_RAINING:
      ...
      break;
    case SKY_LIGHTNING:
      sprintf(buf, "How can you be out on such a miserable %s, %s!", buf2,
              GET_NAME(t));
      break;
    default:
...
struct breather breath_monsters[] = {
  ...
  { 243, 55, breaths + 2 },   { 3670, 30, breaths + 2 },
  { 3674, 45, breaths + 6 },  { 3675, 45, breaths + 8 },
  { 3676, 30, breaths + 6 },  { 3952, 20, breaths + 8 },
  { 5005, 55, breaths + 4 },  { 6112, 55, breaths + 4 },
  { 6635, 55, breaths + 0 },  { 6609, 30, breaths + 0 },
  { 6642, 45, breaths + 2 },  { 6801, 55, breaths + 2 },
  { 6802, 55, breaths + 2 },  { 6824, 55, breaths + 0 },
  { 7040, 55, breaths + 6 },  { 9217, 45, breaths + 4 },
  { 9418, 45, breaths + 2 },  { 9419, 45, breaths + 2 },
  { 15858, 45, breaths + 0 }, { 15879, 30, breaths + 0 },
  { 16620, 45, breaths + 0 }, { 16700, 45, breaths + 4 },
  { 16738, 75, breaths + 6 }, { 18003, 20, breaths + 8 },
  { 20002, 55, breaths + 6 }, { 20017, 55, breaths + 6 },
  { 20016, 55, breaths + 6 }, { 20016, 55, breaths + 6 },
  { 25009, 30, breaths + 6 }, { 25504, 30, breaths + 4 },
  { 27016, 30, breaths + 6 }, { -1 },
};
  ...
  if (!ch->specials.fighting) {
    ...
    if (IS_AFFECTED(vict, AFF_FIRESHIELD) || IS_AFFECTED(vict, AFF_SANCTUARY)) {
      if (GetMaxLevel(ch) >= GetMaxLevel(vict)) {
        act("$n utters the words 'use instaway instant magic remover'", 1, ch,
            0, 0, TO_ROOM);
        cast_dispel_magic(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
        return (TRUE);
      }
    ...
    if (IS_SET(rp->room_flags, NO_SUM) || IS_SET(rp->room_flags, TUNNEL) ||
      ...
      if (rp->sector_type == SECT_FOREST) {
        ...
        if (level > 30 && !number(0, 4)) {
          act("$n utters the words 'Where is my SERVANT!'", FALSE, ch, 0, 0,
              TO_ROOM);
          ...
          if (level > 10 && !number(0, 5)) {
            ...
            cast_animal_summon_1(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, 0,
                                 0);
            return (FALSE);
          }
          if (level > 16 && !number(0, 5)) {
            act("$n whistles loudly", FALSE, ch, 0, 0, TO_ROOM);
            cast_animal_summon_2(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, 0,
                                 0);
            return (FALSE);
          }
          if (level > 24 && !number(0, 5)) {
            act("$n whistles extremely loudly", FALSE, ch, 0, 0, TO_ROOM);
            cast_animal_summon_3(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, 0,
                                 0);
            return (FALSE);
          }
        }
        ...
        if (!IS_SET(rp->room_flags, INDOORS)) {
          if (level > 8 && !number(0, 3)) {
            ...
            cast_control_weather(GetMaxLevel(ch), ch, "worse", SPELL_TYPE_SPELL,
                                 0, 0);
            return (FALSE);
          }
          if (level > 15 && !number(0, 2)) {
            act("$n utters the words 'here lightning'", FALSE, ch, 0, 0,
                TO_ROOM);
...
int druid_challenge_prep_room(struct char_data *ch, int cmd, char *arg,
  ...
  if (cmd == NOD) {
    ...
    if (GET_EXP(ch) <=
        titles[ DRUID_LEVEL_IND ][ GET_LEVEL(ch, DRUID_LEVEL_IND) + 1 ].exp -
            100) {
...
int druid_challenge_room(struct char_data *ch, int cmd, char *arg,
  ...
  if (cmd == FLEE) {
    ...
    if (IS_PC(ch)) {
      ...
      GET_EXP(ch) =
          MIN(titles[ DRUID_LEVEL_IND ][ (int) GET_LEVEL(ch, DRUID_LEVEL_IND) ].exp,
              GET_EXP(ch));
      ...
      if (mob_index[ ch->nr ].virtual >= DRUID_MOB &&
        ...
        for (i = me->people; i; i = i->next_in_room)
          if (IS_PC(i)) {
            ...
            GET_EXP(i) = MAX(
                titles[ DRUID_LEVEL_IND ][ GET_LEVEL(i, DRUID_LEVEL_IND) + 1 ]
                        .exp +
                    1,
                GET_EXP(i));
...
int monk_challenge_room(struct char_data *ch, int cmd, char *arg,
  ...
  if (cmd == FLEE) {
    ...
    if (IS_PC(ch)) {
      ...
      GET_EXP(ch) =
          MIN(titles[ MONK_LEVEL_IND ][ (int) GET_LEVEL(ch, MONK_LEVEL_IND) ].exp,
              GET_EXP(ch));
      send_to_char("Go home.\n\r", ch);
      char_from_room(ch);
      char_to_room(ch, rm - 1);
        ...
        for (i = me->people; i; i = i->next_in_room)
          if (IS_PC(i)) {
            ...
            if (HasClass(i, CLASS_MONK)) {
              GET_EXP(i) = MAX(
                  titles[ MONK_LEVEL_IND ][ GET_LEVEL(i, MONK_LEVEL_IND) + 1 ]
                          .exp +
                      1,
                  GET_EXP(i));
...
int attack_rats(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  int dir;
...
int DragonHunterLeader(struct char_data *ch, int cmd, char *arg,
  ...
  if (type == PULSE_TICK) {
    if (ch->specials.position == POSITION_SITTING) {
      ...
      switch (number(1, 10)) {
      case 1:
        do_emote(ch, "mumbles something about in his day the tavern being a "
                     "popular hangout.",
                 0);
        break;
      case 2:
...
int DwarvenMiners(struct char_data *ch, int cmd, char *arg,
  ...
  if (type == EVENT_END_STRIKE) {
    if (ch->specials.position == POSITION_SITTING) {
      ...
      ch->player.long_descr =
          (char *)realloc(ch->player.long_descr, sizeof(char) * 50);
  ...
  if (type == EVENT_DWARVES_STRIKE) {
    if (ch->specials.position == POSITION_STANDING) {
      ...
      ch->player.long_descr =
          (char *)realloc(ch->player.long_descr, sizeof(char) * 55);
      strcpy(ch->player.long_descr,
  ...
  if (type == PULSE_TICK) {
    ...
    if (ch->generic == 30) { /* strike over, back to work */
      ...
      if (ch->specials.position == POSITION_SITTING) {
        ...
        ch->specials.default_pos = POSITION_STANDING;
        ch->player.long_descr =
            (char *)realloc(ch->player.long_descr, sizeof(char) * 65);
        strcpy(ch->player.long_descr,
               "A dwarven mine-worker is here, working the mines.\n\r");
      }
      ch->generic     = 0;
...
int real_rabbit(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
  struct char_data *i;
