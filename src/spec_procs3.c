...
#define CAN_GO_ROOM(roomp, dir)                                                \
  (EXIT_ROOM(roomp, dir) && real_roomp(EXIT_ROOM(roomp, dir)->to_room))

/* get pointer to room in the given direction */
#define ROOMP(roomp, dir)                                                      \
  ((CAN_GO_ROOM(roomp, dir)) ? real_roomp(EXIT_ROOM(roomp, dir)->to_room)      \
                             : NULL)
    ...
    if (cmd == 1 && ch->in_room == TimnusRoom) {
      if ((TIMNUSNORTHLIMIT < GetMaxLevel(ch)) &&
        ...
        if (!check_soundproof(ch)) {
          act("$n tells you 'Thou art not pure enough of heart.'", TRUE, mob, 0,
              ch, TO_VICT);
        }
...
int DeathRoom(int dt_room) {
  ...
  for (k = real_roomp(dt_room)->contents; k;
       k = real_roomp(dt_room)->contents) {
...
int YouthPotion(struct char_data *ch, int cmd, char *arg, struct char_data *mob,
                int type) {
...
const struct turbolift_keywords TurboLiftList[ 5 ] = { { 3030, "dump" },
                                                       { 3033, "zifnab" },
                                                       { 2, "hell" },
                                                       { 2500, "draagdim" },
                                                       { 0, "\n" } };
...
char *scroll_text[] = {
  ...
  "mistletoe are said to be the cure-all of everything.",
  "White stone in one hand, anchor in the other, holy symbol raised high,\nthe "
  "bark clad druid mumbled something about needing a silver collar.\n",
