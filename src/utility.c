...
void debug(char *s) { log_sev(s, 1); } /*thought this was a prototype - heheh */
...
int CAN_SEE(struct char_data *s, struct char_data *o) {
  ...
  if (IS_AFFECTED(o, AFF_SANCTUARY) || (IS_AFFECTED(o, AFF_FIRESHIELD)) ||
    ...
    if (!rp->tele_targ) {
      ...
      if (!IS_SET(TELE_COUNT, rp->tele_mask)) {
        ...
        fprintf(fp, "%d %ld -1 %d %d %d %d %d", rp->zone, rp->room_flags,
                rp->tele_time, rp->tele_targ, rp->tele_mask, rp->tele_cnt,
                rp->sector_type);
      }
    }
                    ...
                    if (!IS_AFFECTED(ch, AFF_FLYING) ||
                      ...
                      if (!MOUNTED(ch)) {
                        ...
                        if (rp && rp->dir_option[ rd ] &&
                          ...
                          char_to_room(
                              ch, (real_roomp(or))->dir_option[ rd ]->to_room);
                          do_look(ch, "\0", 15);
                          if (RIDDEN(ch)) {
                            do_look(RIDDEN(ch), "\0", 15);
                          }
...
void SpaceForSkills(struct char_data *ch) {
  ...
  */
  ...
  ch->skills = (struct char_skill_data *)malloc(MAX_SKILLS *
